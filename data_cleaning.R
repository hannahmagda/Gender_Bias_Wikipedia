...
cze_alive_text$Word_Count <- sapply(cze_alive_text$plain_text, count_words)
...
clean_data <- function(df) {
  ...
  removal_reason_not_found <- sum(grepl("^(Error fetching content for page:|No Wikipedia page name provided or missing)", df$plain_text, ignore.case = TRUE))
  ...
  df <- df %>%
    filter(!grepl("^(Redirect to:|Weiterleitung nach:|Rediriger vers:|Redirige a:|Přesměrování na:)", plain_text, ignore.case = TRUE) &
             ...
             !grepl("Error fetching content for page:|No Wikipedia page name provided or missing", plain_text, ignore.case = TRUE))
...
#clean the dataframes for each country
