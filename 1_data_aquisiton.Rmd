...
This script provies the code to aquire the necessary Wikipedia Data for my analyis, using the CLD database and the wikiediaR package. It outputs the raw data used for the analysis, namely "deu_text".
...
In order to conduct the analysis, we need the Comparative Legislator Database for Germany which we can simply access via the legislatoR package. The data is read in ans saved in the following code.
```{r}
...
The following code provides the functions necessary to aquire the wikipedia texts, using the page_name of the politicians. 
```{r}
...
de_text_pipeline <- function(page_name) {
  ...
  #best practice to avoid overloading the Wikipedia servers and to practice responsible web scraping
