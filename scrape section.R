...
# Define a function to extract text based on XPath expression
extract_text_from_html <- function(html_content) {
  # Parse the HTML content
  parsed_html <- read_html(html_content)
  
  # Extract text based on the specified XPath expression
  extracted_text <- parsed_html %>%
    html_nodes(xpath = "//h2[span[contains(@class, 'mw-headline') and contains(., 'Leben')]]/following-sibling::p[following-sibling::h2[1][span[contains(@class, 'mw-headline')]]]") %>%
    html_text() %>%
    paste(collapse = "\n")  # Combine multiple paragraphs into a single string
  
  return(extracted_text)
}

df$leben <- sapply(df$text, extract_text_from_html)

print(df$leben)
print(head(df$text, 1))
# Apply the function to each row of the dataframe and store the result in a new column

write.csv(df, file = "raw_data/df_test.csv", row.names = FALSE)
...
extract_text_from_html <- function(html_content) {
  ...
  # Extract text based on the specified XPath expression
  extracted_text <- parsed_html %>%
    html_nodes(xpath = "//h2[span[contains(@class, 'mw-headline') and contains(., 'Leben')]]/following-sibling::*[following-sibling::h2[span[contains(@class, 'mw-headline')]]][not(self::h2)]") %>%
    html_text() %>%
    paste(collapse = "\n")  # Combine multiple paragraphs into a single string
...
library(rvest)
library(magrittr) # Für das %>% Pipe-Symbol
library(dplyr) # Für mutate

# Funktion, die HTML-Inhalt als Input nimmt und den Text zwischen bestimmten Abschnitten extrahiert
extract_text_between_headlines <- function(html_content) {
  # HTML-Inhalt parsen
  parsed_html <- read_html(html_content)
  
  # Das erste <h2>-Element finden, das "Leben und Beruf" enthält
  start_h2 <- html_nodes(parsed_html, xpath = "//h2[span[contains(@class, 'mw-headline') and contains(., 'Leben und Beruf')]]")
  
  # Den Text des nächsten <h2>-Elements finden
  next_h2 <- html_node(start_h2, xpath = "following-sibling::h2[1]")
  
  # Alle <p>-Elemente zwischen dem Start-<h2> und dem nächsten <h2> extrahieren
  if (length(start_h2) > 0 && !is.null(next_h2)) {
    extracted_text <- html_nodes(parsed_html, xpath = sprintf("//h2[span[contains(., 'Leben und Beruf')]]/following-sibling::p[preceding-sibling::h2[1] = %s]", xml_attr(next_h2, 'xpath'))) %>%
      html_text() %>%
      paste(collapse = " ")
  } else if (length(start_h2) > 0) {
    # Wenn es kein nachfolgendes <h2> gibt, extrahiere alle nachfolgenden <p> bis zum Ende
    extracted_text <- html_nodes(start_h2, xpath = "following-sibling::p") %>%
      html_text() %>%
      paste(collapse = " ")
  } else {
    # Kein passendes <h2> gefunden
    extracted_text <- ""
  }
...
# Angenommen, Ihr DataFrame heißt df und die relevante Spalte heißt 'html_content'
# Anwenden der Funktion auf jede Zeile und Speichern des Ergebnisses in einer neuen Spalte 'extracted_text'
df <- df %>%
  dplyr::mutate(extracted_text = sapply(text, extract_text_between_headlines, simplify = FALSE))
...
paragraphs_xpath <- paste0(start_xpath, '/following::p[following::span[@id="Politik"]]')
...
start_xpath <- '/html/body/div[3]/div[3]/div[5]/div[1]/h2[1]/span[1]'
end_xpath <- '/html/body/div[3]/div[3]/div[5]/div[1]/h2[2]/span[1]'
following_xpath <- '/following::p[following::span[@id="Politik"] or following::span[@id="Partei"] or following::span[@id="Beruf"] or following::span[@id="Politische Tätigkeiten
"] or following::span[@id="Partien"]]'

df <- df %>%
  mutate(extracted_text = map_chr(wiki_url, possibly(~extract_paragraphs_text(.x, start_xpath, end_xpath, following_xpath), NA)))
