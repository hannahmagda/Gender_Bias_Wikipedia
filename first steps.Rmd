...
library(ggplot2)
library(stringr) #remove css like structures
...
cze_text_pipeline <- function(page_name) {
  Sys.sleep(runif(1, 1, 2))
  ...
  # Try fetching Wikipedia content
  tryCatch({
    wp_content <- WikipediR::page_content("cs", "wikipedia", page_name = page_name)
    plain_text <- html_text(read_html(wp_content$parse$text$`*`))
    return(plain_text)
  }, error = function(e) {
    return(paste("Error fetching content for page:", page_name))
...
en_text_pipeline <- function(page_name) {
  ...
  tryCatch({
    ...
    if (!is.null(wp_content$parse$text$`*`)) {
      html_content <- wp_content$parse$text$`*`
      
      # HTML-Text in lesbar formatierten Text umwandeln
      plain_text <- html_text(read_html(html_content), trim = TRUE)
...
#  
# 
# gbr_alive_text$Word_Count <- sapply(gbr_alive_text$plain_text, count_words)
#  write.csv(gbr_alive_text, file = "gbr_alive_text.csv")
 
#  irl_alive_text <- irl_core_alive %>%
#    mutate(plain_text = sapply(wikititle, en_text_pipeline))
#  
# 
# irl_alive_text$Word_Count <- sapply(irl_alive_text$plain_text, count_words)
#  write.csv(irl_alive_text, file = "irl_alive_text.csv")
#  
# can_alive_text <- can_core_alive %>%
#    mutate(plain_text = sapply(wikititle, en_text_pipeline))
#  
# 
# can_alive_text$Word_Count <- sapply(can_alive_text$plain_text, count_words)
#  write.csv(can_alive_text, file = "can_alive_text.csv")
#  
#  usa_alive_text <- usa_core_alive %>%
#    mutate(plain_text = sapply(wikititle, en_text_pipeline))
#  
# 
# usa_alive_text$Word_Count <- sapply(usa_alive_text$plain_text, count_words)
#  write.csv(usa_alive_text, file = "usa_alive_text.csv")
...
#   mutate(plain_text = sapply(wikititle, en_text_pipeline))
# 
# sco_alive_text$Word_Count <- sapply(sco_alive_text$plain_text, count_words)
# write.csv(sco_alive_text, file = "sco_alive_text.csv")
...
# write.csv(cze_alive_text, file = "cze_alive_text.csv")
...
sco_alive_text$plain_text <- str_remove_all(sco_alive_text$plain_text, "\\..*?\\{.*?\\}")
usa_alive_text$plain_text <- str_remove_all(usa_alive_text$plain_text, "\\..*?\\{.*?\\}")



#clean data, remove missing/unuseful wikipedia data
sco_alive_text <- sco_alive_text %>%
  filter(!grepl("^Redirect to:", plain_text, ignore.case = TRUE) &
         !grepl("may refer to:", plain_text, ignore.case = TRUE))

usa_alive_text <- usa_alive_text %>%
  filter(!grepl("^Redirect to:", plain_text, ignore.case = TRUE) &
         !grepl("may refer to:", plain_text, ignore.case = TRUE))
...
This research question is interesting to me as I am interested in gender equality topics for several years now, having studies the child penalty, working with the issue of the gender care gap in my student jobs and overall being motivated to learn more about gender related inequalities. Int he scope of my masters thesis, i want to learn more about such issues in the online world, wikipedia is used by over 4 billion unique global visitors a month and the seventh most visited websites worldwide in 2022. Thus, it is very exciting to see, what differences there are between male and female politicians. The category of politicians is specifically interested, being a public policy student, I am particularly intrigued by inequalities in this sphere as poliicians represent all citizens and have a real life impact.
